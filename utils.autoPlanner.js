...
if (!Memory.autoPlanner) Memory.autoPlanner = {}

module.exports = function autoPlanner() {
    let p = Game.flags.p;
    let c = Game.flags.c;
    let pa = Game.flags.pa;
    let pb = Game.flags.pb;
    ...
    if (p && pa && pb && pc && pm) {
        Memory.autoPlanner[p.pos.roomName] = ManagerPlanner.computeManor(p.pos.roomName, [pc, pm, pa, pb]);
        p.remove();
        pa.remove();
        pb.remove();
        pc.remove();
    ...
    // for (roomName in Memory.autoPlanner) {
    //     if (Game.rooms[roomName].controller && !c
    //         && (Game.rooms[roomName].controller.progress / Game.rooms[roomName].controller.progressTotal) < 0.01) {
    //         Game.rooms[roomName].createFlag(25, 25, 'c');
    //     }
    // }
    ...
    if (c) {
        ...
        for (roomName in Memory.autoPlanner) {
            ...
            const roomStructsData = Memory.autoPlanner[roomName];
            HelperVisual.showRoomStructures(roomStructsData.roomName, roomStructsData.structMap);
